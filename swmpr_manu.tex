...
\multicolumn{1}{l}{Function}&\multicolumn{1}{c}{Description}\tabularnewline
\hline
\texttt{aggregate.swmpr}&Aggregate \texttt{swmpr} objects for different time periods - years, quarters, months,  weeks, days, or hours.  The aggregation function defaults to the mean.\tabularnewline
\texttt{aggregate\_metab.swmpr}&Aggregate metabolism data from a \texttt{swmpr} object.  This is primarly used within \texttt{plot\_metab} but may be useful for simple summaries of raw metabolism data.\tabularnewline
...
> aggregate(dat, 'months', params = c('do_mgl'))
\end{verbatim}
\end{kframe}
\end{knitrout}
...
> plot(dat, type = 'l')
> lines(dat_smooth, select = 'do_mgl', col = 'red', lwd = 2)
\end{verbatim}
