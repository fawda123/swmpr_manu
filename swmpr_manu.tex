...
\hline
\texttt{all\_params}&Retrieve up to 100 records\textsuperscript{\textit{a}} starting with the most recent at a given station, all parameters.  Wrapper to \texttt{exportAllParamsXMLNew} function on web services.\tabularnewline
\texttt{all\_params\_dtrng}&Retrieve up to 1000 records of all parameters within a given date range for a station.  Optional argument for a single parameter. Wrapper to \texttt{exportAllParamsDateRangeXMLNew}.\tabularnewline
...
\textsuperscript{\textit{a}} \footnotesize Server limits prevent retrieving more records in a single request.\end{table}
