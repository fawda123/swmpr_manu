...
sort_val <- order(to_plo[to_plo$metab == 'Pg', 'value'])
to_plo$site <- factor(gsub('wq$', '', to_plo$site))
to_plo$site <- factor(
  to_plo$site,
  levels = levels(to_plo$site)[sort_val],
  labels = levels(to_plo$site)[sort_val]
  )
new_labs <- c('Mean net annual metabolism', 'Mean annual gross production', 'Mean annual total respiration')
...
p <- ggplot(to_plo, 
  ...
  facet_wrap( ~ metab, ncol = 1, scales = 'free_y') + 
...
metabs_tab$yr_grp[metabs_tab$year > 2004] <- '2005-2014'
metabs_tab <- mutate(metabs_tab, value = 0.032 * value) %>% 
  group_by(metabs_tab, site, metab, yr_grp)
