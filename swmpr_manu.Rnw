...
% acronyms from glossaries package
\newacronym{CDMO}{CDMO}{Centralized Data Management Office}
\newacronym{CRAN}{CRAN}{Comprehensive R Archive Network}
\newacronym{DO}{DO}{dissolved oxygen}
\newacronym{NERRS}{NERRS}{National Estuarine Research Reserve System}
\newacronym{QAQC}{QAQC}{quality assurance/quality control}
\newacronym{SWMP}{SWMP}{System Wide Monitoring Program}
...
opts_chunk$set(fig.align = 'center', message = F, dev = 'pdf', dev.args = list(family = 'serif'), fig.pos = '!h', warning = F, background = 'white', highlight = FALSE, prompt = TRUE, size = 'small', out.width = '0.00\\textwidth')
...
descrips <- c(
  ...
  'Import files from a local path.  The files must be in a specific format, such as those returned from the \\gls{CDMO} using the zip downloads option.',
...
atts_desc <- c(
  ...
  'Station identifier used by \\gls{NERRS} as a string with 7 or 8 characters.',
  "Character vector of column names for data parameters, e.g., \\texttt{`do\\_mgl'}, \\texttt{`turb'}, etc.",
  'Indicates if \\gls{QAQC} columns are present in the raw data.',
...
foot <- c('\\textsuperscript{a}\\footnotesize Time zones that do not observe daylight savings are used for \\texttt{swmpr} objects and may not be cities in the United States.  For example, ``America/Jamaica" is used for Eastern Standard Time.')
...
descrips <- c(
  ...
  'Remove \\gls{QAQC} columns and remove data based on \\gls{QAQC} flag values for a \\texttt{swmpr} object.  Only applies if \\gls{QAQC} columns are present. ', 
  'View a summary of the number of observations in a \\texttt{swmpr} object that are assigned to each \\gls{QAQC} flag used by \\gls{CDMO}.  The output can be used to inform further processing.', 
  'Remove replicate nutrient data that occur on the same day.  The default is to average replicates.', 
  'Format data from a \\texttt{swmpr} object to a continuous time series at a given timestep.  The function is also used in \\texttt{comb}.',
...
descrips <- c(
  ...
  'Aggregate metabolism data from a \\texttt{swmpr} object.  This is primarly used within \\texttt{plot\\_metab} but may be useful for simple summaries of daily metabolism data.',
  ...
  'Create a map of all stations in a reserve using the ggmap package \\cite{Kahle13}.',
  ...
  'Plot ecosystem metabolism estimates after running \\texttt{ecometab} on a combined \\texttt{swmpr} object.',  
...
<<decomp_ex1, eval = T, fig.height = 6, cache = T, fig.cap = "An additive decomposition of dissolved oxygen into a trend, seasonal (daily), and random component using the \\texttt{decomp} function.">>=
# get data
data(apadbwq)
dat <- apadbwq
...
The following is an example that shows use of the \texttt{ecometab} function from a combined water quality and weather data set.  Monthly aggregations of the raw, daily estimates are plotted using \texttt{plot\_metab} (\Cref{fig:metab_ex}).
...
Several additional functions are provided that do not fit the above categories (\Cref{tab:misc}).  These functions are generally used within the main functions but may be useful for more customized evaluation of \gls{SWMP} data.  
...
descrips <- c(
  ...
  'Returns column names as a list for the parameter types (nutrients, weather, or water quality).  Includes \\gls{QAQC} columns with \\texttt{f\\_} prefix. Used in the data retrieval functions.',
  'Parses HTML returned from \\gls{CDMO} data requests.  Used in the retrieval functions.',
...
latex(
  to_tab[, 'Description', drop = F],
  file = '',
  caption = "Miscellaneous functions available from the SWMPr package.  Most are used within the main functions above but may be useful for customized evaluations of \\gls{SWMP} data.  Full documentation for each function is in the help file (e.g., execute \\texttt{?calckl} at the command line).",
  rowlabel = 'Function',
  colheads = 'Description',
  rowname = to_tab$Functions,
  caption.loc = 'top',
