...
metabs <- foreach(wq_site = wq_sites) %dopar% {
  ...
  if(length(met_site) > 0){
    ...
    # wq
    raw_content <- paste0('https://s3.amazonaws.com/swmpalldata/', wq_site, '.RData')
    ...
    # met
    raw_content <- paste0('https://s3.amazonaws.com/swmpalldata/', met_site, '.RData')
    raw_content <- httr::GET(raw_content)$content
    connect <- rawConnection(raw_content)
    load(connect)
    met <- get(met_site)
    ...
    ## 
    # combine, subset up to 2013, estimate metabolism
    dat <- SWMPr::comb(wq, met, method = met_site)
    dat <- subset(dat, subset = '2014-01-01 0:0', operator = '<')
    dat <- SWMPr::ecometab(dat)
