...
opts_chunk$set(fig.align = 'center', message = F, dev = 'pdf', dev.args = list(family = 'serif'), fig.pos = '!ht', warning = F, background = 'white')
...
cap <- 'Trends in metabolism for two sites at each of the \\ac{NERRS} reserves.  Values are averages of mean annual estimates for each period of observation (1994-2004 and 2005-2014).  Bold values indicate an increase from the first period, whereas italic values indicate a decrease.  Regions are approximations of geographic coordinates and are not exact, e.g., Old Woman Creek (Lake Erie) is Atlantic.' 
