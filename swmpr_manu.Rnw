...
The second approach for data retrieval is to use the \texttt{import\_local} function to import data into R that are locally available after downloading from \ac{CDMO}.  This approach is most appropriate for large, specific data requests. The \texttt{import\_local} function is designed for data from the zip downloads feature in the advanced query section of the \ac{CDMO} website. The zip downloads feature can be used to obtain data from multiple stations in one request.  The downloaded data will be in a compressed folder that includes multiple .csv files by year for a given data type (e.g., apacpwq2002.csv, apacpwq2003.csv, apacpnut2002.csv, etc.).  The \texttt{import\_local} function can be used to import files directly from the zipped folder or after the folder is decompressed.
...
A sample dataset can be downloaded for use with the examples below (see the \nameref{supp_info}).  This dataset has an identical format as the data returned from the zip downloads feature of the \ac{CDMO}.  These data are also included with the package as binary data files (RData) that can be loaded using the \texttt{data} function.  These include \texttt{swmpr} objects for four stations at Apalachicola Bay: apacpnut, apacpwq, apadbwq, and apaebmet.  Information for each file can be viewed in the help documentation (e.g., \texttt{?apacpnut}).
...
\href{https://s3.amazonaws.com/swmpexdata/zip_ex.zip}{https://s3.amazonaws.com/swmpexdata/zip\_ex.zip} A compressed data folder is provided to show the format provided by the zip downloads feature of the \ac{CDMO}.  These data can be retrieved for use in R with the \texttt{import\_local} function.  The same data are included with the SWMPr package as binary RData files. 

\subsection*{\ac{SWMP} data as R binary files}

\href{https://s3.amazonaws.com/swmpalldata/}{https://s3.amazonaws.com/swmpalldata/} All \ac{SWMP} data from 1995 through 2014, one file per site. See the help file for \texttt{import\_remote} for more information.
